defmodule Anticipay.Aggregate do
  use GenServer
  ...
  def handle_cast({:handle, %{"command" => _} = command}, state) do
    Anticipay.EventStore.emit(received(command))
  ...
  defp route_commands(commands) do
    Enum.each(commands, &Anticipay.Aggregates.handle/1) # Anticipay.Transaction.FireAndForget.start(commands)
  end
  defp route_commands(parent, {:fork, commands}), do: Anticipay.Transaction.ForkAndJoin.start(parent, commands)
  defp route_commands(_parent, {:chain, _commands}), do: raise "unimplemented" # Anticipay.Transaction.Chain.start(parent, commands)
  defp route_commands(_parent, {:forward, _command}), do: raise "unimplemented" # Anticipay.Transaction.Forward.start(parent, command)
  ...
  defp commit_events(events) do
    Enum.each(events, &Anticipay.EventStore.emit/1)
  end

  defp received(%{"command" => _, "_id" => id} = command),
    do: %{"event" => "CommandReceived", "command_id" => id, "command" => command, "_id" => Anticipay.UUID.new}

  defp succeeded(%{"_id" => id}),
    do: %{"event" => "CommandSucceeded", "command_id" => id, "_id" => Anticipay.UUID.new}

  defp splitted(%{"_id" => id}),
    do: %{"event" => "CommandHandedOver", "command_id" => id, "_id" => Anticipay.UUID.new}

  defp failed(%{"_id" => id}, reason),
    do: %{"event" => "CommandFailed", "command_id" => id, "why" => reason, "_id" => Anticipay.UUID.new}

  defp rolledback(%{"_id" => id}),
    do: %{"event" => "CommandRolledBack", "command_id" => id, "_id" => Anticipay.UUID.new}
end
