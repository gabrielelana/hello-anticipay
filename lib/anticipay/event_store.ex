defmodule Anticipay.EventStore do
  use GenServer
  ...
  def events_where({:received_after, from}, filter, limit) do
    from = Anticipay.MongoDB.DateTime.from_datetime(from)
  ...
  def events_where({:received_between, from, to}, filter, limit) do
    from = Anticipay.MongoDB.DateTime.from_datetime(from)
    to = Anticipay.MongoDB.DateTime.from_datetime(to)
  ...
  def handle_call({:fetch, query, filter, limit}, _from, state) do
    events =
      Mongo.find(Anticipay.MongoDB, @collection, query, sort: %{"_offset" => 1}, limit: limit)
      |> Enum.to_list
      |> Enum.filter(filter)
      |> Enum.map(&Anticipay.MongoDB.from_document/1)
    {:reply, events, state}
  end
  ...
  defp enrich(event, offset) do
    event
    |> Map.put("_received_at", Anticipay.MongoDB.DateTime.utc_now)
    |> Map.put("_offset", offset)
  end

  defp store(event) do
    Mongo.save_one(Anticipay.MongoDB, @collection, Anticipay.MongoDB.to_document(event))
    event
  end
  ...
  defp offset_of_last_event do
    last_event = Mongo.find(Anticipay.MongoDB, @collection, %{}, sort: %{"_offset" => -1}, limit: 1) |> Enum.to_list
    case last_event do
      [event] -> event["_offset"]
      _ -> 0
    end
